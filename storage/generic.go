...
func reverseFilterEarlyExit(values []string, filter Filter, limit int) []SearchResult {
	results := make([]SearchResult, 0, clampUpfrontCap(limit, len(values)))
...
func applySearchHintsLinear(values []string, hints *SearchHints) []SearchResult {
	...
	case OrderByScoreDesc:
		// Reached only when Limit is 0 (unlimited).
...
// clampUpfrontCap returns a capacity hint suitable for make([]T, 0, cap) when
// the caller knows the result cannot exceed numValues entries: a near-MaxInt
// limit (reachable only when the operator disabled --web.search.max-limit)
// would otherwise crash makeslice. Used by the top-K and reverse-early-exit
// paths in ApplySearchHints, where the result is bounded by the input length.
func clampUpfrontCap(limit, numValues int) int {
	if limit > numValues {
		return numValues
	}
	return limit
...
func linearResultCap(numValues, limit int) int {
	...
	if cap < minLinearAllocCap {
		cap = minLinearAllocCap
	}
	if cap > numValues {
		cap = numValues
	}
...
func topKByScore(values []string, filter Filter, limit int) []SearchResult {
	h := make(searchTopKHeap, 0, clampUpfrontCap(limit, len(values)))
	for _, v := range values {
