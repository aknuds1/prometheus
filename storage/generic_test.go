...
func TestApplySearchHints(t *testing.T) {
	...
	asc := append([]string(nil), values...)
	// Input is the natural index order; for these tests it happens to be
	// the alphabetical order we want when documenting OrderByValueAsc.
	require.Equal(t, []string{"alpha", "beta", "delta", "epsilon", "gamma"}, sorted(asc))
