...
func TestWriteStorageApplyConfig_NoDuplicateWriteConfigs(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run("", func(t *testing.T) {
			s := NewWriteStorage(nil, nil, dir, time.Millisecond, nil, false, nil)
			conf := &config.Config{
...
func TestWriteStorageApplyConfig_UpdateWithRegisterer(t *testing.T) {
	...
	s := NewWriteStorage(nil, prometheus.NewRegistry(), dir, time.Millisecond, nil, false, nil)
...
func TestWriteStorageApplyConfig_Lifecycle(t *testing.T) {
	...
	s := NewWriteStorage(nil, nil, dir, defaultFlushDeadline, nil, false, nil)
	conf := &config.Config{
		GlobalConfig: config.DefaultGlobalConfig,
...
func TestWriteStorageApplyConfig_UpdateExternalLabels(t *testing.T) {
	...
	s := NewWriteStorage(nil, prometheus.NewRegistry(), dir, time.Second, nil, false, nil)
...
func TestWriteStorage_CanRegisterMetricsAfterClosing(t *testing.T) {
	...
	s := NewWriteStorage(nil, reg, dir, time.Millisecond, nil, false, nil)
	require.NoError(t, s.Close())
	require.NotPanics(t, func() { NewWriteStorage(nil, reg, dir, time.Millisecond, nil, false, nil) })
}
