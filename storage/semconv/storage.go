...
func queryLabelValues(ctx context.Context, q labelQuerier, e *schemaEngine, name string, hints *storage.LabelHints, matchers []*labels.Matcher) ([]string, annotations.Annotations, error) {
	...
	// query every historical/escaped alias of `name` across every variant and
	// merge the values; mismatched (variant, alias) pairs simply return nothing.
	// For __name__ there are no attribute aliases (aliasesOf returns just the
	// name), and its values are collapsed to the canonical metric below.
