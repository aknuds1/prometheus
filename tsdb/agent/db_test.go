...
func createTestAgentDB(t testing.TB, reg prometheus.Registerer, opts *Options) *DB {
	...
	rs := remote.NewStorage(promslog.NewNopLogger(), reg, startTime, dbDir, time.Second*30, nil, false, nil)
	t.Cleanup(func() {
...
func TestLockfile(t *testing.T) {
	tsdbutil.TestDirLockerUsage(t, func(t *testing.T, data string, createLock bool) (*tsdbutil.DirLocker, testutil.Closer) {
		...
		reg := prometheus.NewRegistry()
		rs := remote.NewStorage(logger, reg, startTime, data, time.Second*30, nil, false, nil)
		t.Cleanup(func() {
			require.NoError(t, rs.Close())
		})
...
func TestDuplicateSeriesRefsByHash(t *testing.T) {
	dbDir := t.TempDir()
	opts := DefaultOptions()
	rs1 := remote.NewStorage(promslog.NewNopLogger(), nil, startTime, dbDir, time.Second*30, nil, false, nil)
	...
	rs2 := remote.NewStorage(promslog.NewNopLogger(), nil, startTime, dbDir, time.Second*30, nil, false, nil)
