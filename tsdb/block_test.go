...
func TestBlockIndexReader_PostingsForLabelMatching(t *testing.T) {
	testPostingsForLabelMatching(t, 2, func(t *testing.T, series []labels.Labels) IndexReader {
		...
		for _, s := range series {
			seriesEntries = append(seriesEntries, storage.NewListSeries(s, []chunks.Sample{sample{100, 0, nil, nil, nil}}))
		}
	...
	for _, s := range series {
		...
		for ; typ != chunkenc.ValNone; typ = it.Next() {
			...
			case chunkenc.ValFloat:
				t, v := it.At()
				seriesMeta := it.AtSeriesMetadata()
				ref, err = app.Append(ref, lset, t, v, seriesMeta)
			case chunkenc.ValHistogram:
				t, h := it.AtHistogram(nil)
				seriesMeta := it.AtSeriesMetadata()
				ref, err = app.AppendHistogram(ref, lset, t, h, nil, seriesMeta)
			case chunkenc.ValFloatHistogram:
				t, fh := it.AtFloatHistogram(nil)
				seriesMeta := it.AtSeriesMetadata()
				ref, err = app.AppendHistogram(ref, lset, t, nil, fh, seriesMeta)
			default:
...
func createHeadWithOOOSamples(tb testing.TB, w *wlog.WL, series []storage.Series, chunkDir string, oooSampleFrequency int) *Head {
	...
	for i, lset := range oooSampleLabels {
		...
		for _, sample := range oooSamples[i] {
			ref, err = app.Append(ref, lset, sample.T(), sample.F(), sample.SeriesMetadata())
			require.NoError(tb, err)
			oooSamplesAppended++
		}
	}
