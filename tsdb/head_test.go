				...
				func(b *testing.B) {
					...
					if c.mmappedChunkT != 0 {
						...
						for k := 0; k < c.batches*c.seriesPerBatch; k++ {
							...
							s.append(c.mmappedChunkT, 42, nil, 0, cOpts)
...
func TestMemSeries_truncateChunks(t *testing.T) {
	...
	for i := 0; i < 4000; i += 5 {
		ok, _ := s.append(int64(i), float64(i), nil, 0, cOpts)
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.mmappedChunks > 0 {
				...
				for i := 0; i < (tc.mmappedChunks+1)*chunkRange; i += chunkStep {
					ok, _ := series.append(int64(i), float64(i), nil, 0, cOpts)
					require.True(t, ok, "sample append failed")
				}
...
func TestMemSeries_append(t *testing.T) {
	...
	for i := 1; i < 1000; i++ {
		ok, _ := s.append(1001+int64(i), float64(i), nil, 0, cOpts)
		require.True(t, ok, "append failed")
	}
	s.mmapChunks(chunkDiskMapper)
...
func TestMemSeries_appendHistogram(t *testing.T) {
	...
	// on and after it.
	// New chunk must correctly be cut at 1000.
	ok, chunkCreated := s.appendHistogram(998, histograms[0], nil, 0, cOpts)
	require.True(t, ok, "append failed")
	require.True(t, chunkCreated, "first sample created chunk")

	ok, chunkCreated = s.appendHistogram(999, histograms[1], nil, 0, cOpts)
	require.True(t, ok, "append failed")
	require.False(t, chunkCreated, "second sample should use same chunk")

	ok, chunkCreated = s.appendHistogram(1000, histograms[2], nil, 0, cOpts)
	require.True(t, ok, "append failed")
	require.True(t, chunkCreated, "expected new chunk on boundary")

	ok, chunkCreated = s.appendHistogram(1001, histograms[3], nil, 0, cOpts)
	require.True(t, ok, "append failed")
	require.False(t, chunkCreated, "second sample should use same chunk")
	...
	ok, chunkCreated = s.appendHistogram(1002, histogramWithOneMoreBucket, nil, 0, cOpts)
	require.True(t, ok, "append failed")
...
func TestMemSeries_append_atVariableRate(t *testing.T) {
	...
	for i := 0; i < samplesPerChunk/4; i++ {
		ok, _ := s.append(nextTs, float64(i), nil, 0, cOpts)
	...
	ok, chunkCreated := s.append(DefaultBlockDuration, float64(0), nil, 0, cOpts)
...
func TestGCChunkAccess(t *testing.T) {
	...
	ok, chunkCreated := s.append(0, 0, nil, 0, cOpts)
	require.True(t, ok, "series append failed")
	require.True(t, chunkCreated, "chunks was not created")
	ok, chunkCreated = s.append(999, 999, nil, 0, cOpts)
	require.True(t, ok, "series append failed")
	require.False(t, chunkCreated, "chunks was created")

	// A new chunks should be created here as it's beyond the chunk range.
	ok, chunkCreated = s.append(1000, 1000, nil, 0, cOpts)
	require.True(t, ok, "series append failed")
	require.True(t, chunkCreated, "chunks was not created")
	ok, chunkCreated = s.append(1999, 1999, nil, 0, cOpts)
	require.True(t, ok, "series append failed")
	require.False(t, chunkCreated, "chunks was created")
...
func TestHeadReadWriterRepair(t *testing.T) {
	...
	// Fill the chunk segments and corrupt it.
	{
		...
		for i := 0; i < 7; i++ {
			ok, chunkCreated := s.append(int64(i*chunkRange), float64(i*chunkRange), nil, 0, cOpts)
			require.True(t, ok, "series append failed")
			require.True(t, chunkCreated, "chunk was not created")
			ok, chunkCreated = s.append(int64(i*chunkRange)+chunkRange-1, float64(i*chunkRange), nil, 0, cOpts)
			require.True(t, ok, "series append failed")
...
func TestDataMissingOnQueryDuringCompaction(t *testing.T) {
	...
	for i := int64(0); i <= 120*7; i++ {
		...
		require.NoError(t, err)
		maxt = ts
		expSamples = append(expSamples, sample{ts, float64(i), nil, nil, nil})
	}
	require.NoError(t, app.Commit())
	...
	{ // Initial data that goes into snapshot.
		...
		for i := 1; i <= numSeries; i++ {
			...
			for ts := int64(1); ts <= 240; ts++ {
				val := rand.Float64()
				expSeries[lblStr] = append(expSeries[lblStr], sample{ts, val, nil, nil, nil})
				ref, err := app.Append(0, lbls, ts, val, nil)
				require.NoError(t, err)

				hist := histograms[int(ts)]
				expHist[lblsHistStr] = append(expHist[lblsHistStr], sample{ts, 0, hist, nil, nil})
				_, err = app.AppendHistogram(0, lblsHist, ts, hist, nil, nil)
				require.NoError(t, err)

				floatHist := floatHistogram[int(ts)]
				expFloatHist[lblsFloatHistStr] = append(expFloatHist[lblsFloatHistStr], sample{ts, 0, nil, floatHist, nil})
